        ...
        for proc_node_id in range(num_proc):
            ...
            try:
                ...
                if msg_type == "TERMINATE" and msg["receiver_id"] == node_id:
                    ...
                    msg_type == "LEADER"
                    and msg["receiver_id"] == node_id
                    and msg["sender_id"] < node_id
                ):
                    ...
                    SHARED_THREAD_MSGS[node_id]["PUB_SOCKET"].send_string(
                        "RESP", flags=zmq.SNDMORE
                    )
    ...
    if len(quests) == 4:
        ...
        for node_id in alive_nodes:
            if node_id in starter_nodes:
                ...
                starter = False
            proc = Process(
                target=leader, args=(node_id, starter, num_proc, SHARED_THREAD_MSGS)
            )
